...
//        var REMOTE_CORPS_ID = 0; // 远程兵 // @test
        ...
        { // 设置随机上场的 boss 单位
            var corpsCids = TriggerAction.GetInLevelCorpsCids();
            for (var k in corpsCids) {
                var corpsCid = corpsCids[k];
                if (!SOLDIER_CORPS_ID_DIC[corpsCid]) {
                    BOSS_CORPS_ID_LIST.push(corpsCid);
                }
            }
        }
